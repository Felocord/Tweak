...
func swizzleDCDThemeColor(_ semanticColors: [String: [String]]) {
  os_log("Swizzling DCDThemeColor", log: felocordLog, type: .info)
  ...
  os_log("Found instance of DCDThemeColor", log: felocordLog, type: .debug)
  let target: AnyClass = object_getClass(DCDThemeColor)!

  os_log("Found DCDThemeColor", log: felocordLog, type: .debug)

  var methodCount: UInt32 = 0
  let methods = class_copyMethodList(target, &methodCount)!

  os_log("DCDThemeColor has %{public}d methods", log: felocordLog, type: .debug, methodCount)
  ...
  for i in 0..<Int(methodCount) {
    ...
    os_log(
      "Found method %{public}@", log: felocordLog, type: .debug, methodName)
    if let semanticColor = semanticColors[methodName] {
      os_log(
        "Swizzling %{public}@", log: felocordLog, type: .debug, methodName)
...
func swizzleUIColor(_ rawColors: [String: String]) {
  os_log("Swizzling UIColor", log: felocordLog, type: .info)

  let UIColor: AnyClass = NSClassFromString("UIColor")!
  os_log("Found instance of UIColor", log: felocordLog, type: .debug)
  let target: AnyClass = object_getClass(UIColor)!

  os_log("Found UIColor", log: felocordLog, type: .debug)

  var methodCount: UInt32 = 0
  let methods = class_copyMethodList(target, &methodCount)!

  os_log("UIColor has %{public}d methods", log: felocordLog, type: .debug, methodCount)
